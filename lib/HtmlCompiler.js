...
class HtmlCompiler {
  ...
  compile(path) {
    ...
    while ( tags.length > 0 ) {
      ...
      const importPath = this.resolvePath(currdir, this.extractPath(tag));
      if ( this.isHtmlFile(importPath) ) {
        if ( this.tags.template.test(tag) ) {
          let template = this.readHtml(importPath);
          this.findTags(template).forEach(t => tags.push(t));
          const importContent = htmlString.replace(tag, '');
          htmlString = template.replace('{{content}}', importContent);
        }
        else {
          const importContent = this.compile(importPath);
          htmlString = htmlString.replace(tag, importContent);
        }
      }
    }
    return htmlString;
