...
class HtmlCompiler {
  ...
  compile(path) {
    ...
    while ( tags.length > 0 ) {
      ...
      if ( this.isHtmlFile(importPath) ) {
        const regexp = new RegExp(this.tags.template, 'g');
        if ( regexp.test(currtag) ) {
      ...
      else if ( new RegExp(this.tags.asset, 'g').test(currtag) || new RegExp(this.tags.url, 'g').test(currtag) ) {
  ...
  findTags(html) {
    ...
    Object.keys(this.tags).forEach(tag => {
      const regexp = new RegExp(this.tags[tag], 'g');
      console.log(regexp);
      const a = html.match(regexp);
