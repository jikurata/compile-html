...
class HtmlCompiler {
  ...
  compile(path) {
    ...
    while ( tags.length > 0 ) {
      ...
      if ( this.isHtmlFile(importPath) ) {
        ...
        else if ( this.tags.url.test(currtag) ) {
          const newUrl = this.resolveUrl(currdir, importPath);
  ...
  resolveUrl(currdir, path) {
    if ( !this.url ) {
      console.error('A #url() tag was found, but no url was provided to resolve it');
      return path;
    }
    switch(path.charAt(0)) {
      // Resolves relative directory urls
      case '.': return `${this.url}/${this.resolvePath(currdir,path)}`
      // Resolves root relative urls
      case '/': return `${this.url}${path}`;
      // Resolves path relative and absolute urls
      default: {
        if ( path.includes('http') || path.includes('www.') ) return path;
